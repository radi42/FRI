...
\paragraph{}
inerne siete v ramci AS: \say{neighbor (lebo je to nas sused)}, \say{update-source (aby BGP nenadaval na iny interface, ktorym to sirime)}. Pokial bolo v AS viac ako dva smerovače, este sme pridali príkaz \say{next-hop-self}. Router ID sme najprv nenastavovali, ale potom sa nam to vypomstilo, lebo po pridani loopbacku1 sa router ID nastavil prave nan, lebo mal vacsiu IPcku. Po zmene Router ID prikazom \say{bgp router-id 10.255.255.X} (X symbolizuje cislo smerovača) spôsobí rozpad BGP spojenia, ktoré sa po chvíli (rádovo v desiatkach sekúnd) obnoví.
...
\paragraph{}
Privatne siete operatorov ohlasujeme prikazom \say{network}.
...
router bgp 3401
  ...
  network 64.34.1.0 mask 255.255.255.0
...
int lo0
  ...
  no shut
int lo1
  ip addr 200.110.2.1 255.255.255.0
  ip router isis
  no shut
router isis
...
router bgp 110
  ...
  network 200.110.2.0 mask 255.255.255.0
...
int lo0
  ...
  no shut
int lo1
  ip addr 200.110.3.1 255.255.255.0
  ip router isis
  no shut
router isis
...
router bgp 110
  ...
  network 200.110.3.0 mask 255.255.255.0
  no auto-summary
  no sync
  bgp log-neighbor-changes
...
int lo0
  ...
  no shut
int lo1
  ip addr 200.110.4.1 255.255.255.0
  ip router isis
  no shut
router isis
...
router bgp 110
  ...
  network 200.110.4.0 mask 255.255.255.0
  no auto-summary
  no sync
  bgp log-neighbor-changes
...
int lo0
  ...
  no shut
int lo1
  ip addr 128.45.5.1 255.255.255.128
  no shut
router bgp 4502
...
int lo0
  ...
  no shut
int lo1
  ip add 200.33.6.1 255.255.255.128
  ip router isis
router isis
...
int lo0
  ...
  no shut
int lo1
  ip addr 200.33.7.1 255.255.255.128
  ip router isis
  no shut
router isis
...
int lo0
  ...
  no shut
int lo1
  ip add 200.110.12.1 255.255.255.128
  ip router isis
router isis
...
int lo0
  ...
  no shut
int lo1
  ip addr 200.110.13.1 255.255.255.128
  ip router isis
  no shut
router isis
...
int lo0
  ...
  no shut
int lo1
  ip addr 223.255.255.1 255.255.255.128
  no shut
router bgp 5005
...
\paragraph{}
Ale potom sa uz neohlasovali. Z IS-IS sme ich odstranili prikazmi uvdedenymi v casti \say{Popis}.
