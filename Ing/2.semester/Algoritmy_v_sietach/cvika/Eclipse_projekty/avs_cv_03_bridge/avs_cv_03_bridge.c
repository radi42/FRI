...
int main (int argc, char *argv[])
{
  ...
  for(int i = 0; i < argc; i++) {
	  if(ints[i].socket > maxSockNo){
		  maxSockNo = ints[i].socket;
	  }
  }
  ...
  for (;;){
	  ...
	  // struktura pre funkciu select()
	  // predstavuje mnozinu rozhrani (socketov)
	  fd_set FDs;

	  /* Do mnoziny FDs pridame vsetky sockety. */
	  // najprv vynulujeme strukturu fd_set
      FD_ZERO (&FDs);
      ...
      // read() je blokujuca operacia citania
      // select() sleduje dane filedeskriptory (rozhrania) a ak aspon na jedno rozhranie prijde ramec, posle ho dalej.
      // select() zjednocuje riadenie rozhrani
      // musim zistit na ktorom rozhrani som ramec dostal a vykonat operaciu read.
      // v momente ked na as
      select (maxSockNo, &FDs, NULL, NULL, NULL);

      /* Ak udalost nastala, zistime, na ktorych socketoch mi prisiel ramec. Na to sluzi funkcia FD_ISSET
       * Musime iterovat vsetky rozhrania a funkciou FD_ISSET zistovat, ci na danoe rozhranie nieco prislo */
      for (int i = 0; i < argc - 1; i++){
