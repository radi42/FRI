...
int main(int argc, char **argv) {
	...
	sscanf(myMAC,
			"%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx", // hhx - dva znaky bude brat ako jeden byte - FUNGUJE IBA PRE HEXA CISLA
	...
	for (;;) {
		...
		printf("[IP] %s\t\t[MAC] %02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx\n",
				...
				respArpHdr->srcHwAddr[0], respArpHdr->srcHwAddr[1],
				respArpHdr->srcHwAddr[2], respArpHdr->srcHwAddr[3],
				respArpHdr->srcHwAddr[4], respArpHdr->srcHwAddr[5]);
		break;
	}
