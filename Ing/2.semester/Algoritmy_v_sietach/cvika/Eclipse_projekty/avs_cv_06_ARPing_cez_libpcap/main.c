...
void obsluhaRamca(u_char *user, const struct pcap_pkthdr *hdr, const u_char *frame) {
	if(hdr->caplen < (sizeof(struct ethHdr) + sizeof(struct arpHdr))){
		printf("Frame too small (runt)");
		return;
	}
	...
	struct arpHdr *arp_resp = (struct arpHdr *) eth->payload;

	//a potom dereferencujeme premenne struktury ethHdr
	struct in_addr src_ip;
	src_ip.s_addr = arp_resp->srcIP;

	printf("%d\t%d\tResponse from\t%s\tat %02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx\n",
			hdr->ts.tv_sec,
			hdr->caplen,
			inet_ntoa(src_ip),
			*(arp_resp->srcMAC),
