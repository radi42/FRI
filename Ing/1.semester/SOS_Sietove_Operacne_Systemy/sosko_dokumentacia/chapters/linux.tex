...
\paragraph{}
Pre správne fungovanie SLAVE DNS servera pri výpadku MASTER DNS treba SLAVE server autentifikovať voči MASTER serveru kľúčom (viď obr. \ref{dns_views}). Potrebujeme editovať súbor \say{/etc/bind/named.conf.options}. V lokálnej zóne do časti \say{match-clients} pridáme záznam \say{!key fero}, čím vypneme lokálnu autentifikáciu SLAVE servera, pretože lokálne SLAVE server vedel resolvovať. Vo verejnej zóne sme do \say{match-clients} museli pridať záznam \say{key fero}, aby sa zvonku vedel SLAVE server autentifikovať MASTER DNS serveru kľúčom. Kľúč sme generovali rovnako, ako v kapitole \say{Web server} pre HTTPS.
...
\paragraph{}
Uvedenému popisu zodpovedá aj konfiguračný súbor pre DHCP server. Na Serveri 2 sme upravovali súbor \say{/etc/default/isc-dhcp-server}. Definovali sme dva adresné rozsahy: jeden pre servery, druhý pre klientov. Prvý rozsah sme iba definovali, pretože v ňom máme servery so statickými IP adresami. Pre druhý rozsah sme nastavovali parametre ako napr. rozsah prideľovaných adries (\say{range} od 192.168.0.130 - 254), maska (\say{option subnet-mask} - 255.255.255.128), broadcast adresu (\say{option broadcast-address} - 192.168.0.255), predvolenú bránu (\say{option routers} - 192.168.0.129) a IP adresu DNS servera (\say{192.168.0.2}).

\paragraph{}
FW plní úlohu DHCP Relay servera. Všetky DHCP požiadavky od klientov prechá\-dzajú cez FW ku DHCP serveru, ktorý pridelí klientovi IP adresu a ďalšie nakonfigurované informácie. Týmto spôsobom je FW zodpovedný iba za filtrovanie premávky a server za služby poskytované na sieti. IP adresa DHCP servera sa do konfiguračného súboru \say{/etc/default/isc-dhcp-relay} DHCP relay agenta musí zadať ku atribútu \say{SERVERS} BEZ úvodzoviek. Musíme počúvať nielen na podrozhraní, na ktoré sú pripojení klienti, ale aj na podrozhraní, na ktoré je pripojený aj DHCP server t.j. eth1.10 aj eth1.20, ktoré sú definované v atribúte \say{INTERFACES}. Konfiguračný súbor DHCP Relay servera je uvedená nižšie.
...
\paragraph{}
V súbore na serveri s2 \say{/etc/ntp.conf} sme pridali slovenské servery zo stránky \say{www.pool.ntp.org/zone/sk}, ostatné servery sme zakomentovali. 
...
Upravovali sme súbor \say{/etc/ntp.conf}. V ňom sme definovali, pre aké adresné rozsahy sa má čas synchronizovať a z akého servera sa má získavať. Po reštarte NTP služby na serveri aj klientoch príkazom \say{service ntp restart} si klienti z serveru 2 aktualizujú čas.
...
\subsection{Joomla}
\paragraph{}
Najprv je potrebné prejsť do priečinku \say{/var/www/Joomla} a následne stiahneme Joomlu 3.6.2 a potom ju odzipujeme.
...
\paragraph{}
Po týchto krokoch sa môžem pustiť do inštalácie Joomly. Do prehliadača zadáme \say{http://localhost} a prehliadač nás presmeruje na Inštaláciu Joomly 3.6.2.

\begin{figure}[!htb]
\centering
\includegraphics[width=14cm]{linux_web_joomla_1}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=14cm]{linux_web_joomla_4}
\caption{Joomla (dokoncenie instalacie)}
\label{fig:web_joomla_finish}
\end{figure}

\paragraph{}
Je potrebné zmazať inštalačný adresár. Je to možné kliknutím na tlačidlo \say{remove installation folder}, alebo ho môžeme vymazať ručne v adresári say{/var/www/Joomla/installation}.

\subsection{Mediawiki}
\paragraph{}
Do adresára \say{var/www/web2} sme stiahli Wikimedia archív týmito príkazmi
...
\paragraph{}
Prejdeme inštaláciou, kde sme nastavili hostname na \say{sos3.local}. Následne sme museli reštartovať postfix \say{service postfix restart}. V súbore \say{/etc/postfix/main.cf} je potrebné upraviť \say{myhostname = sos3.local}, odkomentovať \say{myorigin = \$myhostname}. Taktiež bolo potrebné odkomentovať \say{inet\_interfaces = all} a \say{inet\_protocols = all} a pridať \say{mynetworks = 192.168.0.0/24}. \say{Mail\_spool\_directory = /var/spool/mail} je cesta ku mailom pre administrátora. Následne nám po reštarte servera vypísalo started postfix: ok.
