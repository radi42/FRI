...
void sortComplexArray(double complex *pPole, int paSize, int paAscending)
{
    // Selection sort - najdi najmensie a daj ho na zaciatok
    int i, j, position;
    double complex swap;
    ...
    for ( i = 0 ; i < paSize - 1 ; i++ )
    {
        ...
        for ( j = i + 1 ; j < paSize ; j++ )
        {
            if(paAscending == 1)
            {
                ...
                if ( cabs(pPole[position]) > cabs(pPole[j]) )
                {
                    position = j;
                }
            }
...
//    double reMax = 2.2;
    ...
    // Triedenie od najmensieho po najvacsi
    //sortComplexArray(pole, velkostPola, 1);
    // Triedenie od najvacsieho po najmensi
    sortComplexArray(pole, velkostPola, 0);
