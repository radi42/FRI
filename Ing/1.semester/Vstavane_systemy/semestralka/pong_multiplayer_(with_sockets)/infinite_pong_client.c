...
#include "infinite_pong_client.h"
...
int main(int argc, char *argv[])
{
    ...
    Game *pong = malloc(sizeof(Game));     // Deklaruj hru / okno
    ...
    char *buffer = malloc(BUFFER_SIZE);      // Velkost buffera je 14, aby sa don zmestili aj zaverecne spravy. Buffer uklada klavesu stlacenu klientom. Ta sa potom ulozi do spravy a odosle serveru, ktory vykona akciu spojenu so stlacenou klavesou
    strcpy(buffer, "start");
...
//    printf("%d;%d;%d;%d;%d;%d\n", pong->window_width, pong->window_height, player_1->width, player_1->height, ball->width, ball->height);
    ...
    if (scoreboard_font == NULL) {
        printf("Nepodarilo sa nacitat pismo: %s \n", TTF_GetError());
        return 400;
    }
...
//        printf("%d;%d;%d;%d;%d;%d || msg = %d\n", player_1->x, player_1->y, player_2->x, player_2->y, ball->x, ball->y, msg->gameStatus);
        ...
        {
            memset(buffer, '\0', BUFFER_SIZE);
            strncpy(buffer, "Koniec hry - server\n", 20);
            printToTerminal(buffer, BUFFER_SIZE);
