...
struct RTEntry *
FindExisting (struct RTEntry *Table, unsigned int Address, unsigned int Mask)
{
  struct RTEntry *I;
...
struct RTEntry *
AddEntry (struct RTEntry *Table, unsigned int Address, unsigned int Mask)
{
  struct RTEntry *I;
...
int
PrintRTable (struct RTEntry *Table)
{
  struct RTEntry *I;
  ...
  while (I != NULL)
    {
      printf ("%hhu.%hhu.%hhu.%hhu/%hhu.%hhu.%hhu.%hhu\n",
	      ...
	      I->Address & 0xFF,
	      (I->Mask >> 24) & 0xFF,
	      (I->Mask >> 16) & 0xFF, (I->Mask >> 8) & 0xFF, I->Mask & 0xFF);
...
void
FlushRTable (struct RTEntry *Table)
{
  struct RTEntry *I, *J;
...
int
GenerateNetworks (struct RTEntry *Table, int Count)
{
  if (Table == NULL)
    return -ERR_NOTCREAT;
...
int
CountRTable (struct RTEntry *Table)
{
  int Count = 0;
  struct RTEntry *I;
...
*/
  ...
  printf
    ("done.\nTime: %ld sec %ld usec, hits: %d, ratio: %f, avg lookup: %f usec, table size: %d\n",
