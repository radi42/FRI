 ...
 */
package tables.NonsortedSequenceTable;
...
import tables.Sorts.ISortableTable;
 ...
 */
public class NonsortedSeqenceTable<K,E> implements ISortableTable<K,E>{
    ...
    public void swap(int paPosition1, int paPosition2) throws ETable {
        if(paPosition1 != paPosition2){
            try {
                TablePair<K,E> pair = aTableElements.get(paPosition1);
                aTableElements.set(paPosition1, aTableElements.get(paPosition2));
                aTableElements.set(paPosition2, pair);
            } catch (EList ex) {
                throw new ETable("swap failed " + ex.getMessage());
            }
        }
    }
    ...
    public void move(int paFromPosition, int paToPosition) throws ETable {
        try {
            TablePair<K,E> pair = aTableElements.get(paFromPosition);
            aTableElements.insert(pair, paToPosition);
        } catch (EList ex) {
            throw new ETable("move failed " + ex.getMessage());
        }
    ...
    public K getKeyAtPosition(int paPosition) throws ETable {
        try {
            return aTableElements.get(paPosition).getKey();
        } catch (EList ex) {
            throw new ETable("get key failed " + ex.getMessage());
        }
    }
    ...
    public E getElementAtPosition(int paPosition) throws ETable {
        try {
            return aTableElements.get(paPosition).getElement();
        } catch (EList ex) {
            throw new ETable("get element failed " + ex.getMessage());
        }
    }
