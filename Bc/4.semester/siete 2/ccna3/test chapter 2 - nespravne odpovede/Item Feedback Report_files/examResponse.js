...
function process(action) {
  ...
  if (action == "go") { 
    ...
    if (result == "intro" || result == "submit" || result == "grade") {
      action = result;
    } 
    else {
...
function ping() {
	window.document.getElementById('ping').src="exam.shtml?BUTTON=ping";
}
...
function displayTimeLeft() {
  ...
  if (timeLeft <= 600000 && timeLeft > 599000) { 
    window.open("exam_alert.shtml?BUTTON=warning", "","toolbar=no,location=no,directories=no,resizable=yes,menubar=no,scrollbars=yes,width=850,height=420");
  }
