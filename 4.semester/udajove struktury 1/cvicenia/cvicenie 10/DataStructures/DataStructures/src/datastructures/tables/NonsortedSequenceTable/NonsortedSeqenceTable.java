...
public class NonsortedSeqenceTable<K,E> implements ITable<K,E>{
    ...
    public E delete(K paKey) throws ETable {
        int index = indexOfKey(paKey);
        try {
            //return (index != -1 ? aTableElements.deleteFromIndex(index).getElement() : null);
            if(index != -1){
                E deleted = aTableElements.get(index).getElement();
                aTableElements.deleteFromIndex(index);  //efektivnejsie: vymen posledny prvok s tym, ktory sa maze, a vymaz posledny; kedze je to neutriedene; je to takto napisane aby sme to mohli pouzit aj na array- aj na linkedlist
                return deleted;
            }
            else{
    ...
    public void modify(K paKey, E paNewElement) throws ETable {
        ...
        try {
            if(containsKey(paKey)){
                ...
                throw new ETable("chyba modifyu: kluc nexistuje ");
            }
        } catch (EList ex) {
    ...
    protected int indexOfKey(K paKey) throws ETable{
        ...
        for (int i = 0; i < size() -1; i++)
            if(aTableElements.get(i).getKey() == paKey) //== porovnava vonkajsok (miesto v pamati), equals porovnava aj vnitornosti
                return i;
        }
        catch (EList ex){
