...
public class KWayTreeNode<E> implements ITreeNode<E>{
    ...
    private int aCardinality;  //kardinalita - maximalne kolko potomkov moze mat rodic

    public KWayTreeNode(E paData, int paCardinality) {
        //this.aData = aData;
        aData = paData;
        aParent = null;
    ...
    public void addChild(ITreeNode<E> paChild) throws ETree {
        try {
            if(degree() == aCardinality)
                throw new ETree("Maximum amount of children reached ");
            else
