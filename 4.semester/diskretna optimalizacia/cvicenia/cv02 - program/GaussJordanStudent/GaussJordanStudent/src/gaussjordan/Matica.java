        ...
        for(int i=0; i<pocRia; i++) {
            //a vsetky stlpce
             for (int j=0; j<pocStl+1; j++) {
                 //ak je prvok v riadku rozny od nuly
                 if(pole[i][j]!= 0) {
                    //ak je pocet stlpcov uz dosiahnuty (nulovy riadok)
                    if(j == pocStl) {
                        baza[i] = -1;
                        return;
                    }
                    //uloz index stlpca pivota do bazy
                    baza[i] = j;
                    double pivot = pole[i][j];
                    //urob pivotovu transformaciu
                    for(int k = 0;k < pocStl+1;k++) {
                        pole[i][k] /= pivot;
                    }
                    for(int k = 0;k < pocRia ;k++) {
                        //urob pre kazdy iny riadok ako pivotovy
                        //pripocitaj opacnu hodnotu prvku pod pivotom k nasledujucemu riadku
                        if(k!=i) {
                            double a = pole[k][j]*(-1);
