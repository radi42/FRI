...
/**
 * Trieda Batoh - algoritmy dualnej a vymennej heuristiky
 * @author Andrej Šišila
 */
public class Batoh {
    
    private int[] aVysledneRiesenie;              //vektor pozostávajúci z 0 a 1 na n-tom mieste predstavuje ci sa n-ty predmet nachadza v batohu
    private Prvok[] aPrvky;                       //samotné pole prvkov
    private int aKapacitaBatohu;                  //K=15000
    private int aCelkovyPocPrvkov;                //n=500
    private int aMaxPocPrvkovVBatohu;             //r=300
    ...
    private int dajAktualHmotPrvkovVBatohu(){
        return aAktualHmotPrvkovVBatohu;
    }
    
    private int dajAktualCenaPrvkovVBatohu(){
        return aAktualCenaPrvkovVBatohu;
    }
    
    private int dajAktualPocPrvkovVBatohu(){
        return aAktualPocPrvkovVBatohu;
    }
    
    private int dajPrevis(){
        return aPrevis;
    }
    
    private int dajVolnaKapacita(){
        return -aPrevis;
    }
    ...
    public String dajStav(){
        ...
        riesenie += String.format("%s %d %n", "Volna kapacita = ", dajVolnaKapacita() );
        return riesenie;
    }
     ...
     */
    private void nacitaj (int paPocetPrvkov) {
     ...
     * Optimalne pripustne riesenie z dualnej heuristiky je vstupom pre vymennu heuristiku
...
//                    hmotnost =  - ;
                    jeVymenaPripustna = aZaradene.get(i).dajHmotnostPrvku() >= aNezaradene.get(j).dajHmotnostPrvku();
